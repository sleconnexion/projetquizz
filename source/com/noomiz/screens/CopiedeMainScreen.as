...
//			sc:ScrollContainer=new ScrollContainer();
			var zz:BitmapData=new BitmapData(55,50,false,0xFF0000);
			
			//var montest_bitmap:Bitmap=new Bitmap(zz,"auto",true);
			var montest_bitmap:Bitmap=new Bitmap(zz);
			
			var txt:Texture=Texture.fromBitmap(montest_bitmap);
			var txt2:Texture=Texture.fromBitmapData(zz);
			
			var tim:TiledImage=new TiledImage(txt,4);
///			tim.setSize(500,500);
			addChild(tim);
...
//			var id= FlexGlobals.topLevelApplication.parameters.monid;
			...
			labelhaut=new Label();
			labelhaut.text="Connais tu vraiment l'OM ?";
			labelhaut.y=100;
			labelhaut.x=10;
			labelhaut.nameList.add(OMDesktopTheme.ALTERNATE_NAME_OM_CUSTOM_BIGLABEL);
			labelhaut.alpha=1;
			...
			xloader.load ( new URLRequest ("http://www9.noomiz.com/om/jsongetquizz/fbid/"+FBUserFactory.getUser().idfb) );
